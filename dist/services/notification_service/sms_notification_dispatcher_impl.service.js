...
class SmsNotificationDispatcherImplementation {
    dispatch(notificationChannel, notification, monitoringService) {
        return __awaiter(this, void 0, void 0, function* () {
            monitoringService.upsertNotificationChannel(new notification_status_model_1.NotificationStatus(notification.id, "pending", undefined, "SMS"));
            ...
            for (let recipient of notificationChannel.recipients) {
                try {
                    ...
                    console.log(error);
                    monitoringService.upsertNotificationChannel(new notification_status_model_1.NotificationStatus(notification.id, "failed", undefined, "SMS"));
                    results.push(false);
                }
            }
            if (results.every(result => result)) {
                monitoringService.upsertNotificationChannel(new notification_status_model_1.NotificationStatus(notification.id, "delivered", undefined, "SMS"));
                return new result_util_1.Success(true);
            }
            return new result_util_1.Success(false);
        });
    }
}
